           ...
           unsigned short dBitsPerSample,unsigned dOptions)
{
  ...
  if(hAudio<0) {
#ifdef DEBUG
    fprintf(stderr,"unable to open %s for capture\n",sAudioDev);
#endif  // DEBUG
    return -1;
  }
           ...
           unsigned short dBitsPerSample,unsigned dOptions)
{
...
#ifdef DEBUG
    fprintf(stderr,"invalid format tag: %d\n",dFormatTag);
#endif  // DEBUG
    return -1;
  }
  /* Set sampling format & bits per sample */
  switch(dBitsPerSample) {
	...
	if(dRequest!=AFMT_U8) {
	  close(hFilename);
#ifdef DEBUG
	  fprintf(stderr,"unsigned eight bit samples unsupported\n");
#endif  // DEBUG
	  return -1;
	}
	break;
	...
	if(dRequest!=AFMT_S16_LE) {
	  close(hFilename);
#ifdef DEBUG
	  fprintf(stderr,"signed sixteen bit samples unsupported\n");
#endif  // DEBUG
	  return -1;
	}
	break;
...
int RecordWavFile(char *sFilename,char *sAudio,int dSeconds,
		  ...
		  unsigned dOptions,uid_t owner,gid_t group)
{
  int hAudio;
  int ret;

  hAudio=open(sAudio,O_RDONLY);
  if(hAudio<0) {
#ifdef DEBUG
    fprintf(stderr,"unable to open device %s for capture\n",sAudio);
#endif  // DEBUG
    return -1;
  }
  ret=RecordWavDesc(sFilename,hAudio,dSeconds,
		    wavHeader,wavProcess,dOptions,owner,group);
  close(hAudio);
  return ret;
}
		  ...
		  unsigned dOptions,uid_t owner,gid_t group)
{
   ...
   * Write the wav file
   */
  hFilename=CreateWav(sFilename,wavHeader,owner,group);
  if(hFilename<0) {
    return -1;
  }
...
int WriteTimerChunk(char *sFilename,struct wavTimer *wavTimer,int size)
{
  int i;
  int fd;
  int total_size;
  ...
  if((fd=OpenWav(sFilename,&wavHeader))<0) {
    printf("Can't Open!\n");
    return FALSE;
  }
  lseek(fd,4,SEEK_SET);
  read(fd,buffer,4);
  total_size=buffer[0]+256*buffer[1]+65536*buffer[2]+16777216*buffer[3];
  timer_size=size*5;
  lseek(fd,4,SEEK_SET);
  WriteDword(fd,total_size+timer_size+8);
  lseek(fd,0,SEEK_END);
  write(fd,"timr",4);
  WriteDword(fd,timer_size);
  for(i=0;i<size;i++) {
    write(fd,&(wavTimer[i].cLabel),1);
    WriteDword(fd,wavTimer[i].dTime);
  }
  close(fd);
  return TRUE;
}
...
struct wavTimer *ReadTimerChunk(char *sFilename,int *size)
{
  int i;
  int fd;
  struct wavHeader wavHeader;
  ...
  strcpy(wavChunk.sName,"timr");
  if(FindChunk(fd,&wavChunk)!=0) {
    close(fd);
    return NULL;
  }
  *size=wavChunk.oSize/5;
  chunk_data=(char *)malloc(wavChunk.oSize);
  lseek(fd,wavChunk.oOffset+8,SEEK_SET);
  read(fd,chunk_data,wavChunk.oSize);
  wavTimer=(struct wavTimer *)malloc(*size*sizeof(struct wavTimer));

  for(i=0;i<*size;i++) {
    wavTimer[i].cLabel=0xFF&chunk_data[i*5];
    wavTimer[i].dTime=(0xFF&chunk_data[i*5+1])+256*(0xFF&chunk_data[i*5+2])+
      65536*(0xFF&chunk_data[i*5+3])+16777216*(0xFF&chunk_data[i*5+4]);
  }
  free(chunk_data);
  close(fd);
  return wavTimer;
